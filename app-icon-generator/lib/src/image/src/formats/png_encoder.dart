...
class PngEncoder extends Encoder {
  ...
  void addFrame(Image image) {
    ...
    if ((image.isHdrFormat && image.format != Format.uint16) || (image.bitsPerChannel < 8 && !image.hasPalette && image.numChannels > 1)) {
    ...
    final filteredImage = Uint8List((image.width * image.height * nc * channelBytes) + image.height);
  ...
  void _writePalette(Palette palette) {
    if (palette.format == Format.uint8 && palette.numChannels == 3 && palette.numColors == 256) {
  ...
  int _filterUp(Uint8List row, Uint8List? prevRow, int bpc, Uint8List out, int oi) {
  ...
  int _filterAverage(Uint8List row, Uint8List? prevRow, int bpc, int bpp, Uint8List out, int oi) {
  ...
  int _filterPaeth(Uint8List row, Uint8List? prevRow, int bpc, int bpp, Uint8List out, int oi) {
