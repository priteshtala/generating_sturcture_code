...
import 'package:app_icon_generator/src/archive/archive.dart';
...
class TiffImage {
  ...
  TiffImage(InputBuffer p) {
    ...
    switch (photometricType) {
      ...
      case TiffPhotometricType.palette:
        if (samplesPerPixel == 1 && colorMap != null && (bitsPerSample == 4 || bitsPerSample == 8 || bitsPerSample == 16)) {
      ...
      case TiffPhotometricType.yCbCr:
        if (compression == TiffCompression.jpeg && bitsPerSample == 8 && samplesPerPixel == 3) {
          imageType = TiffImageType.rgb;
        } else {
  ...
  Image decode(InputBuffer p) {
                                    ...
                                    : isInt && bitsPerSample == 32
                                        ...
                                        : bitsPerSample == 16
                                            ...
                                            : bitsPerSample == 32
                                                ? Format.uint32
                                                : Format.uint8;
    final hasPalette = colorMap != null && photometricType == TiffPhotometricType.palette;
    final numChannels = hasPalette ? 3 : samplesPerPixel;

    final image = Image(width: width, height: height, format: format, numChannels: numChannels, withPalette: hasPalette);
    ...
    if (hasPalette) {
      ...
      for (var i = 0; i < numColors; ++i) {
        p.setRgb(i, cm[colorMapRed + i], cm[colorMapGreen + i], cm[colorMapBlue + i]);
      }
    }
        ...
        for (var x = 0, px = outX; x < tileWidth && px < width; ++x, ++px) {
          if (samplesPerPixel == 1) {
            if (sampleFormat == TiffFormat.float) {
              ...
              var sample = 0;
              if (bitsPerSample == 8) {
                sample = sampleFormat == TiffFormat.int ? byteData.readInt8() : byteData.readByte();
              } else if (bitsPerSample == 16) {
                sample = sampleFormat == TiffFormat.int ? byteData.readInt16() : byteData.readUint16();
              } else if (bitsPerSample == 32) {
                sample = sampleFormat == TiffFormat.int ? byteData.readInt32() : byteData.readUint32();
              }
            ...
            var alpha = 0;
            if (bitsPerSample == 8) {
              gray = sampleFormat == TiffFormat.int ? byteData.readInt8() : byteData.readByte();
              alpha = sampleFormat == TiffFormat.int ? byteData.readInt8() : byteData.readByte();
            } else if (bitsPerSample == 16) {
              gray = sampleFormat == TiffFormat.int ? byteData.readInt16() : byteData.readUint16();
              alpha = sampleFormat == TiffFormat.int ? byteData.readInt16() : byteData.readUint16();
            } else if (bitsPerSample == 32) {
              gray = sampleFormat == TiffFormat.int ? byteData.readInt32() : byteData.readUint32();
              alpha = sampleFormat == TiffFormat.int ? byteData.readInt32() : byteData.readUint32();
            }
            ...
            if (sampleFormat == TiffFormat.float) {
              ...
              var b = 0;
              if (bitsPerSample == 8) {
                r = sampleFormat == TiffFormat.int ? byteData.readInt8() : byteData.readByte();
                g = sampleFormat == TiffFormat.int ? byteData.readInt8() : byteData.readByte();
                b = sampleFormat == TiffFormat.int ? byteData.readInt8() : byteData.readByte();
              } else if (bitsPerSample == 16) {
                r = sampleFormat == TiffFormat.int ? byteData.readInt16() : byteData.readUint16();
                g = sampleFormat == TiffFormat.int ? byteData.readInt16() : byteData.readUint16();
                b = sampleFormat == TiffFormat.int ? byteData.readInt16() : byteData.readUint16();
              } else if (bitsPerSample == 32) {
                r = sampleFormat == TiffFormat.int ? byteData.readInt32() : byteData.readUint32();
                g = sampleFormat == TiffFormat.int ? byteData.readInt32() : byteData.readUint32();
                b = sampleFormat == TiffFormat.int ? byteData.readInt32() : byteData.readUint32();
              }
            ...
            if (sampleFormat == TiffFormat.float) {
              ...
              if (bitsPerSample == 8) {
                ...
                g = sampleFormat == TiffFormat.int ? byteData.readInt8() : byteData.readByte();
                b = sampleFormat == TiffFormat.int ? byteData.readInt8() : byteData.readByte();
                a = sampleFormat == TiffFormat.int ? byteData.readInt8() : byteData.readByte();
              } else if (bitsPerSample == 16) {
                r = sampleFormat == TiffFormat.int ? byteData.readInt16() : byteData.readUint16();
                g = sampleFormat == TiffFormat.int ? byteData.readInt16() : byteData.readUint16();
                b = sampleFormat == TiffFormat.int ? byteData.readInt16() : byteData.readUint16();
                a = sampleFormat == TiffFormat.int ? byteData.readInt16() : byteData.readUint16();
              } else if (bitsPerSample == 32) {
                r = sampleFormat == TiffFormat.int ? byteData.readInt32() : byteData.readUint32();
                g = sampleFormat == TiffFormat.int ? byteData.readInt32() : byteData.readUint32();
                b = sampleFormat == TiffFormat.int ? byteData.readInt32() : byteData.readUint32();
                a = sampleFormat == TiffFormat.int ? byteData.readInt32() : byteData.readUint32();
              }
  ...
  void _jpegToImage(Image tile, Image image, int outX, int outY, int tileWidth, int tileHeight) {
  ...
  void _decodeBilevelTile(InputBuffer p, Image image, int tileX, int tileY) {
    ...
    if (compression == TiffCompression.packBits) {
      ...
      LzwDecoder().decode(InputBuffer.from(p, length: byteCount), byteData.buffer);
      ...
      byteData = InputBuffer(Uint8List(tileWidth * tileHeight));
      try {
        TiffFaxDecoder(fillOrder, tileWidth, tileHeight).decode1D(byteData, p, 0, tileHeight);
      } catch (_) {}
    } else if (compression == TiffCompression.ccittFax3) {
      byteData = InputBuffer(Uint8List(tileWidth * tileHeight));
      try {
        TiffFaxDecoder(fillOrder, tileWidth, tileHeight).decode2D(byteData, p, 0, tileHeight, t4Options!);
      } catch (_) {}
    } else if (compression == TiffCompression.ccittFax4) {
      byteData = InputBuffer(Uint8List(tileWidth * tileHeight));
      try {
        TiffFaxDecoder(fillOrder, tileWidth, tileHeight).decodeT6(byteData, p, 0, tileHeight, t6Options!);
      } catch (_) {}
    } else if (compression == TiffCompression.zip) {
      final data = p.toList(0, byteCount);
      final outData = const ZLibDecoder().decodeBytes(data);
...
enum TiffImageType { bilevel, gray4bit, gray, grayAlpha, palette, rgb, rgba, yCbCrSub, generic, invalid }
